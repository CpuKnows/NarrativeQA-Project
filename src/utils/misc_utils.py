...
def print_out(s, f=None, new_line=True):
  ...
  # stdout
  out_s = s.encode("utf-8", 'ignore')
  if not isinstance(out_s, str):
    out_s = out_s.decode("utf-8", 'ignore')
  print(out_s.encode('ascii', 'ignore'), end="", file=sys.stdout)
...
def format_spm_text(symbols):
  ...
  return u"".join(format_text(symbols).decode("utf-8", 'ignore').split()).replace(
      u"\u2581", u" ").strip().encode("utf-8", 'ignore')
